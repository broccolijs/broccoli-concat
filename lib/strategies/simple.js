...
class Entry {
  constructor(file, contentLimit, fs) {
  ...
  get content() {
    ...
    let content = this._fs.readFileSync(`${this.file}`, 'utf-8');
...
class SimpleConcat {
  constructor(attrs) {
    ...
    this._internalHeaderFiles = this.headerFiles.map(file => { return { file: file, content: '' }; });
    this._internalFooterFiles = this.footerFiles.map(file => { return { file: file, content: '' }; });
  }
  ...
  addFile(file, content) {
    ...
    let entry = new Entry(file, this.contentLimit, this.fs);
